...
namespace BastionSecurityWP\Tests\Unit {
    ...
    {
        $harness = $this->admin();
        $_POST = ['target' => 'critical_settings_alerts', 'command' => 'save', '_wpnonce' => 'valid\\<ignored>', 'enabled' => '1', 'recipients' => 'alerts@example.test'];
        $harness['admin']->handleRequest();
        self::assertTrue($harness['option']['enabled']);

        $harness = $this->admin();
        $_POST = ['target' => 'critical_settings_alerts', 'command' => 'save', '_wpnonce' => ['valid'], 'enabled' => '1', 'recipients' => 'alerts@example.test'];
        $harness['admin']->handleRequest();
        self::assertSame(['enabled' => false, 'recipients' => []], $harness['option']);
