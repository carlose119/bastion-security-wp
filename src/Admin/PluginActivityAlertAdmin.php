...
final class PluginActivityAlertAdmin
{
    ...
    public function renderToolSection(string $notice = ''): void
    {
        ...
        echo '<section id="bastion-plugin-activity-alerts" class="bastion-tools bastion-plugin-activity-alerts"><h2>' . \esc_html__('Plugin activity email alerts', 'bastion-security') . '</h2>';
        ...
        echo '<p>' . \esc_html__('This opt-in, per-site tool will attempt to send plain-text notices for plugin installations and successful activations. Plugin updates are excluded.', 'bastion-security') . '</p>';
        echo '<p>' . \esc_html__('Bastion sends one email per recipient so addresses are not disclosed to each other. An enabled setting means Bastion will attempt to send; it does not prove delivery.', 'bastion-security') . '</p>';
        ...
        echo '<fieldset><legend class="screen-reader-text">' . \esc_html__('Plugin activity email alert settings', 'bastion-security') . '</legend>';
        echo '<label><input type="checkbox" name="enabled" value="1"' . ($state['enabled'] ? ' checked' : '') . '> ' . \esc_html__('Enable plugin activity email alerts', 'bastion-security') . '</label>';
        echo '<p><label for="bastion-plugin-alert-recipients"><strong>' . \esc_html__('Recipients', 'bastion-security') . '</strong></label></p>';
        ...
        echo '<p id="bastion-plugin-alert-recipients-help" class="description">' . \esc_html__('Enter email addresses separated by commas or new lines. Every address must be valid. Enabling requires at least one recipient.', 'bastion-security') . '</p>';
        echo '<p>' . \esc_html__('Disabling preserves the configured recipient list for a later re-enable.', 'bastion-security') . '</p>';
        \submit_button(\esc_html__('Save plugin activity alert settings', 'bastion-security'));
    ...
    private function renderNotice(string $notice): void
    {
        $message = match ($notice) {
            'enabled' => \__('Plugin activity email alerts were enabled.', 'bastion-security'),
            'disabled' => \__('Plugin activity email alerts were disabled. Configured recipients were preserved.', 'bastion-security'),
            'unchanged' => \__('Plugin activity email alert settings were already in the requested state.', 'bastion-security'),
            'invalid_recipients' => \__('Every recipient must be a valid email address. No change was made.', 'bastion-security'),
            'recipients_required' => \__('Add at least one valid recipient before enabling alerts.', 'bastion-security'),
            ...
            'invalid_command' => \__('The requested plugin activity alert target or command is not supported. No change was made.', 'bastion-security'),
            'forbidden' => \__('You are not allowed to perform this action. No change was made.', 'bastion-security'),
            'write_failed' => \__('WordPress could not save the plugin activity email alert settings. The prior state may remain.', 'bastion-security'),
            default => null,
        };
        if ($message === null) {
            return;
