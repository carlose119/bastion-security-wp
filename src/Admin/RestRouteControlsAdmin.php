...
final class RestRouteControlsAdmin
{
    ...
    public function renderCatalog(string $notice = ''): void
    {
        ...
        echo '<section id="bastion-rest-route-controls" class="bastion-tools bastion-rest-route-controls"><h2>' . \esc_html__('REST Route Controls', 'cerrojo-security-toolkit') . '</h2>';
        $this->renderNotice($notice);
        if (! $catalog['available']) {
            echo '<div class="notice notice-error inline"><p>' . \esc_html__('The active REST catalog is unavailable. Normal saving is disabled; Clear all remains available for recovery.', 'cerrojo-security-toolkit') . '</p></div>';
            $this->renderClearForm();
            echo '</section>';
            return;
        }

        echo '<p>' . \esc_html__('Select registered REST route templates and methods to block for ALL users and integrations. Dynamic templates match every concrete URL accepted by WordPress.', 'cerrojo-security-toolkit') . '</p>';
        ...
        echo '</div>';
        echo '<p>' . \esc_html__('The active REST catalog is loaded only on this tab and while validating a save. Discovery calls rest_get_server()->get_routes(), which initializes the request-local registry, fires rest_api_init/rest_endpoints, and materializes route options. It never invokes endpoint or permission callbacks.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Namespaces are collapsed for scanning; selected groups open automatically. Use browser Find to locate a route. Routes remain discoverable because this policy does not remove or hide them.', 'cerrojo-security-toolkit') . '</p>';
        ...
        foreach ($catalog['groups'] as $group) {
            ...
            foreach ($group['routes'] as $route) {
                ...
                if ($route['pairs'] === []) {
                    echo '<p>' . \esc_html__('No configurable methods (OPTIONS and unknown methods are omitted).', 'cerrojo-security-toolkit') . '</p>';
                }
                foreach ($route['pairs'] as $pair) {
                    $id = 'bastion-rest-' . substr(hash('sha256', $pair['token']), 0, 16);
                    echo '<label for="' . \esc_attr($id) . '"><input id="' . \esc_attr($id) . '" type="checkbox" name="rules[]" value="' . \esc_attr($pair['token']) . '"' . ($pair['selected'] ? ' checked' : '') . '> ' . \esc_html($pair['method']) . ($pair['selected'] ? ' — ' . \esc_html__('Selected', 'cerrojo-security-toolkit') : '') . '</label> ';
                }
                echo '</fieldset>';
            }
            echo '</details>';
        ...
        if ($catalog['stale'] !== []) {
            echo '<details class="bastion-rest-namespace" open><summary>' . \esc_html__('Stale selected rules', 'cerrojo-security-toolkit') . ' (' . count($catalog['stale']) . ')</summary>';
            echo '<p>' . \esc_html__('These saved templates are no longer in the active registry. Keep them checked to retain them or uncheck them to remove them.', 'cerrojo-security-toolkit') . '</p>';
            foreach ($catalog['stale'] as $rule) {
                $id = 'bastion-rest-' . substr(hash('sha256', $rule['token']), 0, 16);
                echo '<label for="' . \esc_attr($id) . '"><input id="' . \esc_attr($id) . '" type="checkbox" name="rules[]" value="' . \esc_attr($rule['token']) . '" checked> <code>' . \esc_html($rule['route_pattern']) . '</code> ' . \esc_html($rule['method']) . ' — ' . \esc_html__('Selected (stale)', 'cerrojo-security-toolkit') . '</label>';
            }
            echo '</details>';
        }
        echo '<div class="notice notice-warning inline"><p><strong>' . \esc_html__('Compatibility warning:', 'cerrojo-security-toolkit') . '</strong> ' . \esc_html__('New selections can break administration, site features, and integrations for every identity. Keep an independent non-REST rollback path.', 'cerrojo-security-toolkit') . '</p></div>';
        echo '<p><label><input type="checkbox" name="acknowledgement" value="1"> ' . \esc_html__('I understand the impact of every newly selected method and route template.', 'cerrojo-security-toolkit') . '</label></p>';
        echo '<p>' . \esc_html__('A maximum of 100 selected rules is allowed. GET and synthesized HEAD are independent selections. OPTIONS is not configurable. Query strings are outside the matched request route.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Blocking occurs before permission and endpoint callbacks, after earlier authentication/validation work. Earlier rest_pre_dispatch responses, later filters, direct callback invocation, non-REST paths, and edge caches remain outside this control.', 'cerrojo-security-toolkit') . '</p>';
        \submit_button(\esc_html__('Save REST Route Controls', 'cerrojo-security-toolkit'));
        echo '</form>';
        $this->renderClearForm();
        echo '</section>';
    }
    ...
    {
        return match ($key) {
            'namespaces' => \__('Namespaces', 'cerrojo-security-toolkit'),
            'templates' => \__('Route templates', 'cerrojo-security-toolkit'),
            'pairs' => \__('Selectable pairs', 'cerrojo-security-toolkit'),
            'selected' => \__('Selected', 'cerrojo-security-toolkit'),
            'stale' => \__('Stale', 'cerrojo-security-toolkit'),
            default => '',
        };
    }
    ...
    {
        echo '<h3>' . \esc_html__('Emergency rollback', 'cerrojo-security-toolkit') . '</h3><p>' . \esc_html__('Clear all saved REST route rules without loading the catalog. Deactivation also stops enforcement but preserves the option.', 'cerrojo-security-toolkit') . '</p>';
        echo '<form method="post" action="' . \esc_url(($this->adminUrl)('admin-post.php')) . '">';
        $this->hiddenFields('clear');
        \submit_button(\esc_html__('Clear all REST Route Controls', 'cerrojo-security-toolkit'), 'secondary');
        echo '</form>';
    }
    ...
    private function renderNotice(string $notice): void
    {
        $message = match ($notice) {
            'updated' => \__('REST Route Controls were updated.', 'cerrojo-security-toolkit'),
            'unchanged' => \__('REST Route Controls were already in the requested state.', 'cerrojo-security-toolkit'),
            'acknowledgement_required' => \__('A compatibility acknowledgement is required before adding route rules.', 'cerrojo-security-toolkit'),
            'invalid_rules' => \__('The selected route rules are invalid. No change was made.', 'cerrojo-security-toolkit'),
            'too_many_rules' => \__('More than 100 route rules were submitted. No change was made.', 'cerrojo-security-toolkit'),
            'catalog_unavailable' => \__('The active REST catalog is unavailable. No change was made.', 'cerrojo-security-toolkit'),
            ...
            'invalid_command' => \__('The requested REST route target or command is not supported. No change was made.', 'cerrojo-security-toolkit'),
            'forbidden' => \__('You are not allowed to perform this action. No change was made.', 'cerrojo-security-toolkit'),
            'write_failed' => \__('WordPress could not save and verify the REST route rules. The displayed state is authoritative.', 'cerrojo-security-toolkit'),
            default => null,
        };
        if ($message === null) {
            return;
