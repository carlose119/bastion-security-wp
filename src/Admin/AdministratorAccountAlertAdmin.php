...
final class AdministratorAccountAlertAdmin
{
    ...
    public function renderToolSection(string $notice = ''): void
    {
        ...
        echo '<section id="bastion-administrator-account-alerts" class="bastion-tools bastion-administrator-account-alerts"><h2>' . \esc_html__('Administrator account alerts', 'cerrojo-security-toolkit') . '</h2>';
        ...
        echo '<p>' . \esc_html__('This opt-in, per-site tool observes successful WordPress lifecycle hooks for these exact events: Administrator role granted, Administrator role removed, and Administrator account deleted.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Messages contain the target user ID and bounded login, the administrator role where applicable, contextual current user ID and login, site name and URL, and WordPress-local timestamp. They exclude email addresses, display names, IP addresses, user agents, passwords, capability lists, deletion reassignment, and arbitrary metadata.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('The contextual current user may be unavailable and does not prove causality. Cerrojo sends one plain-text email per recipient so addresses are not disclosed to each other.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Only the current site context is observed. On multisite, remove_user_from_blog may bypass the role-removal hook through remove_all_caps. Cross-site fan-out, network deletion, and super-admin grants and revocations are outside this tool.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Cerrojo attempts wp_mail delivery independently for each recipient, continues after a failed send, does not retry, and does not prove delivery. The tool does not block or roll back account changes.', 'cerrojo-security-toolkit') . '</p>';
        ...
        echo '<fieldset><legend class="screen-reader-text">' . \esc_html__('Administrator account alert settings', 'cerrojo-security-toolkit') . '</legend>';
        echo '<label><input type="checkbox" name="enabled" value="1"' . ($state['enabled'] ? ' checked' : '') . '> ' . \esc_html__('Enable administrator account alerts', 'cerrojo-security-toolkit') . '</label>';
        echo '<p><label for="bastion-administrator-alert-recipients"><strong>' . \esc_html__('Recipients', 'cerrojo-security-toolkit') . '</strong></label></p>';
        ...
        echo '<p id="bastion-administrator-alert-recipients-help" class="description">' . \esc_html__('Enter email addresses separated by commas or new lines. Every address must be valid. Enabling requires at least one recipient.', 'cerrojo-security-toolkit') . '</p>';
        echo '<p>' . \esc_html__('Disabling preserves the configured recipient list for a later re-enable. Delete the saved option to remove the configuration completely.', 'cerrojo-security-toolkit') . '</p>';
        \submit_button(\esc_html__('Save administrator account alert settings', 'cerrojo-security-toolkit'));
    ...
    private function renderNotice(string $notice): void
    {
        $message = match ($notice) {
            'updated' => \__('Administrator account alert settings were updated.', 'cerrojo-security-toolkit'),
            'unchanged' => \__('Administrator account alert settings were already in the requested state.', 'cerrojo-security-toolkit'),
            ...
            'invalid_command' => \__('The requested administrator account alert target or command is not supported. No change was made.', 'cerrojo-security-toolkit'),
            'forbidden' => \__('You are not allowed to perform this action. No change was made.', 'cerrojo-security-toolkit'),
            'write_failed' => \__('WordPress could not save the administrator account alert settings. The prior state may remain.', 'cerrojo-security-toolkit'),
            default => null,
        };
        if ($message === null) {
            return;
